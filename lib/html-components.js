...
HTMLComponents.prototype = {
  ...
  processHTML: function (html) {
    ...
    scriptNodes.forEach(function (scriptNode) {
      ...
      var shouldProcess = /^(?:[^#<]*(<[\w\W]+>)[^>]*$|#([\w\-]*)$)/.test(html) || hasTagChildren;

      if (shouldProcess) {
        var htmlToParse = html;

        // If we have tag children but no text content with HTML, serialize the children
        if (hasTagChildren && !hasCDATA && !/^(?:[^#<]*(<[\w\W]+>)[^>]*$|#([\w\-]*)$)/.test(html)) {
          // Serialize only the tag children (not text nodes)
          var tagChildren = scriptNode.children.filter(function (child) {
            return child.type === "tag";
          });
          htmlToParse = render(tagChildren, { xmlMode: false, decodeEntities: false });
        }
        ...
        if (hasCDATA && cdataNode) {
          ...
          // Replace tag children with processed HTML
          var fragment = parseDocument("<div>" + newHTML + "</div>", {
            xmlMode: true,
            decodeEntities: false,
            lowerCaseTags: false,
            lowerCaseAttributeNames: false,
          ...
          if (wrapperDiv && wrapperDiv.children) {
            scriptNode.children = wrapperDiv.children;
            // Update parent references
            scriptNode.children.forEach(function (child) {
              child.parent = scriptNode;
            });
          }
        } else {
          // Replace text content for scripts with only text children
